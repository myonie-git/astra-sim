...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="将扁平化梯度按 bucket 切分并分别 all-reduce（更接近 DDP 的多次通信）。0 表示不切分。",
    )
    ...
    def run_one_iter(record: bool, iter_idx: int) -> None:
        ...
        if bucket_mb > 0:
            ...
            elem_bytes = int(flat.element_size())
            bucket_elems = max(1, bucket_bytes // elem_bytes)
            total_elems = int(flat.numel())

            b = 0
            for start in range(0, total_elems, bucket_elems):
                end = min(total_elems, start + bucket_elems)
                chunk = flat[start:end]
                chunk_bytes = int(chunk.numel() * elem_bytes)
                ...
                # average
                chunk.div_(float(world_size))

                if record:
                    et.add_allreduce(
                        name=f"iter{iter_idx}_allreduce_bucket{b}",
                        duration_us=comm_us,
                        comm_size_bytes=chunk_bytes,
                    )
                b += 1
        else:
            _sync(device)
